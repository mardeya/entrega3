...
```{r}
db <- db %>% 
  rename("HDI_level" = "hdicode", "Region" = "region", "HDI_rank" = "hdi_rank_2021", 
         "CO2_prod" = "co2_prod_2021", "GNIPC" = "gnipc_2021", "HDI" = "hdi_2021", "Life_exp" 
         = "le_2021")
```

```{r}
db <- db %>% 
  select(1,2,4,3,5,8,6,7,9,10)
```


```{r}
db <- db %>% 
  mutate(HDI_level = factor(HDI_level), Region = factor(Region))
